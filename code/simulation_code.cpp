...
const int min_bits_simulated = 10000000;     // Minimum number of information bits to simulate - high SNR case where convergence is hard
const int min_errors_required = 60000;      // Minimum number of errors before stopping - 150 for high SNR 2.2 dB compensation
const int max_num_rounds = 100000;        // Maximum simulation rounds per SNR point
const double confidence_threshold = 0.1;  // Stop when 95% confidence interval is within 10% of mean - appropriate
...
double phi_func(double Lji)
{
     double result;
     double beta_ji;
     double tmp;
     beta_ji = fabs(Lji); // gets absolute value of floating point number
     if( beta_ji < pow(10,-16)) 
     result = 37.42995; // appropriate
     else if ( beta_ji > 37.43) 
     result = 1.110223e-16; // avoid zeros
     else 
     {
          tmp=(exp(beta_ji)+1)/(exp(beta_ji)-1); // sign follows directly from textbook
          tmp=fabs(tmp);
          result= log(tmp);
     }
     return result;
...
private:
    ...
    void loadConnectivity() {
        ...
        for (int i = 0; i < H_rows; i++) {
            ...
            cn_degrees[i] = degree; // saves number of vns connected to cn i
        }
        fin1.close();
        ...
        for (int j = 0; j < H_cols; j++) {
            ...
            vn_degrees[j] = degree; // saves number of cns connected to vn j
        }
        fin2.close();
    }
...
public:
    ...
    void decode(double* received, int* v) {
        ...
        for (int iter = 0; iter < max_iter; iter++) {
            ...
            // CN update: compute messages from CNs to VNs
            for (int i = 0; i < H_rows; i++) {
                for (int k = 0; k < cn_degrees[i]; k++) { // up to the number of vns connected to cn i
                    int vn_idx = cn_neighbors[i][k];
                    if (vn_idx == -1) break;
                    
                    // Find corresponding message from VN to CN
                    int vn_msg_idx = -1;
                    for (int m = 0; m < vn_degrees[vn_idx]; m++) {
                        if (vn_neighbors[vn_idx][m] == i) {
                            vn_msg_idx = m;
                            break;
                        }
                    }
                    
                    if (vn_msg_idx == -1) continue;
                    
                    // Compute product of all other messages
                    double sum_phi = 0.0;
                    ...
                    for (int m = 0; m < cn_degrees[i]; m++) {
                        if (m != k) {
                            ...
                            // Find the message from other_vn to current CN
                            int other_msg_idx = -1;
                            ...
                            if (other_msg_idx != -1) {
                                sum_phi += phi_func(fabs(L_ji[other_vn][other_msg_idx]));
                                product_sign *= (L_ji[other_vn][other_msg_idx] >= 0) ? 1.0 : -1.0;
                            }
                        }
                    }
                    
                    L_ij[i][k] = product_sign * phi_func(sum_phi);
                }
            }
            ...
            for (int j = 0; j < H_cols; j++) {
                ...
                for (int k = 0; k < vn_degrees[j]; k++) {
                    ...
                    if (cn_msg_idx != -1) {
                        total_llr += L_ij[cn_idx][cn_msg_idx]; // acc from all cns connected
                    }
                }
                ...
                for (int k = 0; k < vn_degrees[j]; k++) {
                    ...
                    if (cn_msg_idx != -1) {
                        L_ji[j][k] = total_llr - L_ij[cn_idx][cn_msg_idx]; // subtract to be sent to vn j - DNC
                    } else {
                        L_ji[j][k] = total_llr;
                    }
                }
...
public:
    ...
    void addErrorRate(double rate) {
        error_rates.push_back(rate); // replace the ber value at the beginning with the latest
    ...
    bool hasConverged() {
        ...
        double variance = 0.0;
        for (double rate : error_rates) {
            variance += (rate - mean) * (rate - mean); // variance formula
        }
        variance /= (error_rates.size() - 1);
        double std_dev = sqrt(variance);
        
        // 95% confidence interval half-width with z value of 1.96 directly from table
        double confidence_interval = 1.96 * std_dev / sqrt(error_rates.size()); // mean 0
        
        // Check if confidence interval is within threshold of mean
        return (confidence_interval / mean) < confidence_threshold; // simplified checker
    }
};
...
public:
    ...
    SimulationResults runSimulation() {
        SimulationResults results = {0, 0, 0, 0.0, false, 0.0};
        auto start_time = tic();
        
        StatisticsTracker stats_tracker;
        int error_check_interval = 50; // Check convergence every 50 rounds 
        
        std::cout << "Simulating Eb/N0 = " << std::fixed << std::setprecision(1) << Eb_N0_dB << " dB..." << std::endl;
        
        for (int round = 0; round < max_num_rounds; round++) {
            // Generate random message
            int message[message_len] = {0};
            for (int i = 0; i < message_len; i++) {
                message[i] = (Normal() < 0.0) ? 1 : 0; //mimic half input probability of 0 or 1
            }
            ...
            results.rounds_completed = round + 1; // DNC in future - the counting is correct
            
            // Check convergence periodically
            if ((round + 1) % error_check_interval == 0) {
                double current_ber = (double)results.total_errors / results.bits_simulated; // accumulated errors and bits_simulated to calculate ber
                ...
                // For very low BER (high SNR), relax the error requirement if we have enough bits - important for ~10^-7
                if (results.bits_simulated >= min_bits_simulated * 10 && results.total_errors >= 50) { // more than 100 million
...
int main() {
    ...
    double Eb_N0_dB_values[] = {1.8};
    int num_snr_points = sizeof(Eb_N0_dB_values) / sizeof(Eb_N0_dB_values[0]); // better in case we want to simulate more points other than 12
    ...
    std::cout << std::string(82, '=') << std::endl; // end with double line
